...
> 判单正常但排行榜显示 0 走 [`判单正常但排行榜显示0.md`](判单正常但排行榜显示0.md)；
> 不算可续分母 / 可续锁在别的订单走 [`不算可续分母或可续锁在别的订单.md`](不算可续分母或可续锁在别的订单.md)；
> 判单开关溯源与回溯补数走 [`判单开关溯源与回溯补数据.md`](判单开关溯源与回溯补数据.md)。
...
| 判单正常、业绩已落库，但**排行榜/业绩显示 0** | **取数问题，不是判单问题** | [`判单正常但排行榜显示0.md`](判单正常但排行榜显示0.md) |
| 不算可续分母 / 可续锁在别的订单 | 可续标记问题 | [`不算可续分母或可续锁在别的订单.md`](不算可续分母或可续锁在别的订单.md) |
| 已确认 `is_attribution=0`，要查谁改的 / 要回溯补数据 | 配置与回溯 | [`判单开关溯源与回溯补数据.md`](判单开关溯源与回溯补数据.md) |
| 对完日志要跳源码 | —— | [`references/代码入口地图.md`](references/代码入口地图.md) |
...
对完日志要跳源码 → [`references/代码入口地图.md`](references/代码入口地图.md)（四条主干 + 两个读码坑）。
...
- 说「这单不进排行榜」之前，必须做完 [`判单正常但排行榜显示0.md`](判单正常但排行榜显示0.md) 的第 6 节 SQL 对账 + 第 7 节打线上接口，两边数字相等才算闭环。
...
见 [`../案例/2026-09-14-判断可续是否要求后置课程下已建班.md`](../案例/2026-09-14-判断可续是否要求后置课程下已建班.md)（已解决：可续只要求续班关系配置存在，不要求后置课程下已建班）。
