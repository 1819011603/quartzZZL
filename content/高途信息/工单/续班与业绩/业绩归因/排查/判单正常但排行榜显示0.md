...
> **只解决**：判单侧一切正常、业绩已落库，但 CRM 排行榜/业绩显示是 0。**这不是判单问题，是取数问题。**
> 业绩表查不到这单 / 归属人为空走 [`查不到归属人.md`](查不到归属人.md)，与本文无关。

这一类**不是判单问题**，是**取数链路问题**。判单侧一切正常，业绩也落库了，但落在了排行榜数据源之外的表里。

## 1. ⛔ 开工第一句话：先问「排行榜右上角的时间筛选选的是哪个区间」
...
拿到区间后**用那个区间打接口**（见第 7 节），别用自己顺手编的区间。

## 2. 结论先行：非课程类订单（物理商品/OMO）的业绩进不了排行榜
...
## 4. ⚠️ 红鲱鱼：`[ItemRouter] no template matched`（8027 是**故意**不匹配的）
...
### 4.1 为什么 physical 永远进不了宽表（两处硬证据）
...
做完 ①②③ 还**不能收工**：必须再做第 7 节（打线上接口复现）+ 第 6 节（SQL 与接口数字对账）。
只有数字对上了，「这单不进排行榜」才是证据，否则只是读代码得出的猜想。

## 6. ★ 必做：打线上接口 + 与 `_total` 表对账（别只靠读代码推演）
...
AskUserQuestion 要一个明确的「授权」再动手，被拦时不要绕，可先用第 6 节的 SQL 对账顶上。

## 验证

排行榜显示 0 的可能原因，按排查顺序：
1. 业绩落在 `performance_physical_attribution`（本文主因，非课程类订单）
2. 时间区间内该老师本就没单（第 6 节）
...
给业务方的口径：「业绩判对了、归属人是本人、金额也对，落在**物理商品业绩表**里；但 CRM 业绩排行榜的数据源只包含课程类订单业绩，所以非课程类订单（如冲刺营这类商品单元）不会计入排行榜。这是取数链路的覆盖缺口，**不是数据丢失，也不能通过回溯补** —— 要修得让 OMO 判单也发 `OrderAttributionPaidEvent`／把 `performance_physical_attribution` 纳入宽表构造，属需求改动。」
