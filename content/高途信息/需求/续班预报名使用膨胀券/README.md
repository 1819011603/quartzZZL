...
## 🔴 与 PRD 的两处冲突（等用户拍板，别自行按 PRD 改）

2026-09-11 按 PRD 对齐校验文案时发现，**PRD 有两条与现有实现/已定共识直接矛盾**，
均已向用户提出、尚未定稿，**在定稿前保持现状**：

| # | PRD 原文 | 现状 | 为什么没直接按 PRD 改 |
|---|---|---|---|
| 1 | 「列表范围：只能选择**【待开始、使用中】**状态的膨胀券」 | 只查【使用中】 | 电商 jar **没有「待开始」这个状态**（1.3.17 = 1 使用中/2 已失效/3 审核中），售卖期由 `saleStartTime`/`saleEndTime` 表达。**09-09 正是照 PRD 这句把状态码搞错过两次**，才定下「只认 jar 不认 PRD 表述」。推测 PRD 的「待开始」≈ 现在的 `saleStatus` 维度（未到开售时间）|
| 2 | 「**每个膨胀券只能用在一个预报名活动中**」，提示【…勾选的膨胀券已被使用】 | **完全没做这条校验**；实现是「活动内唯一、跨活动放行」 | 与「已定共识」里 R-01（王永诗确认**一张券可跨多活动用**）直接冲突。按 PRD 要补一条跨活动占用校验，按共识则不该做 |

> 这两条定了之后：① 记得回头改「已定共识」里对应那条；② `apis.md` 的校验行同步更新。
...
- ❌ ~~`creator` 恒为 null 且无解~~ —— **2026-09-11 已作废**。电商 1.3.17 新增了
  `creatorEmployeeId`，已接上并转成姓名展示（见「已定共识」）。
- 🔴 **券有两个状态字段，别混用**（1.3.17 起）：`couponStatus` 是**券本身**是否生效
  （1 使用中 / 2 已失效 / 3 审核中），`saleStatus` 是**券商品**在不在卖（1 停售中 / 2 开售中，
  平台券为空）。一张券可以是【使用中】但商品【停售中】。
  🔥 **`saleStatus` 与售卖期(`saleStartTime`/`saleEndTime`)是两回事，互不推导**（2026-09-11 实测）：
  三张 caseSDD 券售卖期 09-08~09-30、当时**正在期内**，`sale_status` 仍是 1 停售中 ——
