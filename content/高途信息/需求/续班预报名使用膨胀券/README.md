...
| Apollo | 配置商品类型、入口内容、C 端样式；**student-center 展示白名单已于 2026-09-16（T-43）删除，无需再配**（cart 侧同名 key 仍在用）。完整 key 表见上线 checklist「三、1」 |
| 代课权限 | 登记 `/renewal/pre/coupon/list`、`/renewal/pre/couponScope/list` |
| 反射桥 | ⚠️ **2026-09-20 OCR 评审 + 决定**：`AclServiceCompareController` 是 promotion/cart/product-server **本次新增**的文件，默认 `enabled:true`、暴露任意 Bean 方法。**已定不回退**（测试联调要用），在 Apollo 置 `AclServiceCompareController.enabled=false`（PROD）：product-server-c（`product`）、product-server-b（`product-b`）、cart（`cart.gaotu100.com`）、promotion-b（`promotion.gaotu100.com`）草稿均已建待发布；promotion-c（`gaotu-promotion-c`）无权限（owner litao04）待授权 |
