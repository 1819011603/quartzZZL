...
| 当前卡点 | 缺少真实售罄且使用中的券；缺少有限班容且已满班的班级 |
| 最近更新 | 2026-09-14：排查 B 端券列表返空——product-b 交集正常，命中券 `saleStatus=1`(停售中) 被默认白名单滤掉，属配置口径生效；排查口径已写入 [[verify]] |

## 下一步

1. 造 `sold_count >= total_amount && couponStatus=1` 的券，验证 B 端不可选和 promotion 保存拦截。
2. 造 `capacity>0 && signUpCount>=capacity` 的班级，验证满班拦截，并回归 `capacity=-1` 放行。
3. 复验 `holdLimit=0` 返回“不限”文案。
4. 补 order/cart 下单算价等未覆盖的自动化测试。
5. 上线前完成 DDL、Apollo、代课权限，并把三处反射桥开关显式设为 `false`。
