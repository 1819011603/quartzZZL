...
| T-43 | OES 选品弹窗（复用 `/renewal/pre/coupon/list`）补售卖状态筛选、去白名单 | 进行中 | 部署中，待联调实测 | 2026-09-16 PRD 变更：①筛选区加「使用状态」「售卖状态」多选，**不传即全量**（默认勾选交给前端决定，服务端不再补默认值——2026-09-16 用户订正）；②列表字段加这两列（响应早已有，未改）；③只有【使用中且售卖中】可勾选，其余状态可查看不可选。用户确认接口就是 T-36 的 `/renewal/pre/coupon/list`（OES 弹窗与老师下单弹窗复用同一接口），**不新建接口**。已实现：`PreOrderCouponListRequest` 加 `saleStatusList`，`obtainStatusList()`/`obtainSaleStatusList()` 改为「不传返回 null（全量）」；`PreOrderCouponBiz` 删掉 Apollo 白名单；新增 `revokeSelectableWhenNotOnSaleInUse` 按 `saleStatus` 补 selectable 判定。单测 18 条全过（`PreOrderCouponBizScopeFilterTest`）。已编译、已提交（student-center `49a2a9ab7`）、已推送，正在发布到 `test-gtbg-dev-3`（pipeline 1272566）。细节见 [[apis]]「T-43 落地」。 |
