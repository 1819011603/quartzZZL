...
| 最近更新 | 2026-09-21：README 按需求归档规范瘦身（详细设计归飞书反讲）；tasks 补「完成判据」 |
...
- [ ] 【数据落表】"续班 / 退费所有字段"清单，含 GAIA 动态字段如何落表 —— 细评
...
- [ ] 【AI 配置化】部门 key 口径（整条路径 contains vs 二级部门 code）—— 评审
...
- **别把 `TAG_Subclazz_Transfer` 当调课调班**：它是转辅导班（同班换班主任）；真正的调课调班是 `gaotu_after_sale_event_test` + `TRANSFER_TOUCH_EVENT`。
