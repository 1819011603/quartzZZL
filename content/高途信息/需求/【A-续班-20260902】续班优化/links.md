...
| 类型 | 标题 | 链接 | 记录日期 |
|---|---|---|---|
| 需求目录（我建，个人空间） | 需求 | https://gaotuedu.feishu.cn/wiki/VcOgwHkfeijOM5knBiAcbrLlnY6 | 2026-09-18 |
...
| 原始需求wiki节点 | 续班&退费优化：数据落表 | https://gaotuedu.feishu.cn/wiki/ZEerwf5ZMi9B0Ok9nopcgvoenl2 | 2026-09-17 |
| 原始需求wiki节点 | 续班&退费优化：AI分析模块配置化 | https://gaotuedu.feishu.cn/wiki/AH5ewehsEiXohfkDgQEcKQjHnad | 2026-09-17 |
...
- 问卷匹配优化 wiki token：`Ijz3w9ZgAi8ncakFs3jcKZI9nxi`（document_id: `ILXidWPl5occSSxa6qdcaiosnfe`）
- 主讲数据+下单优化 wiki token：`D89iwm9U1iL6vWkEOgwcg8otnqc`（document_id: `S8IsdepiCo3WasxjLsGcZvsYnQb`）
- 扩科推荐优化 wiki token：`LYCYwqS7ziljApknUlbclPSrnzh`（document_id: `UsFSdlfxjovjOmxEqhLcuHwZnwg`）
- 数据落表 wiki token：`ZEerwf5ZMi9B0Ok9nopcgvoenl2`（document_id: `InPJdfxzcol34ixhLfFcw1DKnsg`）
- AI模块配置化 wiki token：`AH5ewehsEiXohfkDgQEcKQjHnad`（document_id: `Cea4d57Ueo5uoxxwiEvcLjmNn0c`）
- ⚠️ **订正 2026-09-18**：此前主讲/扩科/数据落表 3 个 wiki node token 互相错配（node_token 与 document_id 对不上），已按 `wiki +node-get` 实测改正；document_id 原本是对的。
...
- 我建的需求目录 node token：`VcOgwHkfeijOM5knBiAcbrLlnY6`（obj_token `WVl4dF1O1oRonOxHFlhcroPingf`）
