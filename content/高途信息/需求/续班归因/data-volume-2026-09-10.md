...
- [x] 2,621 个班只出 1 个班的归因 —— **已查清**，见
      [filter-analysis-2026-09-10.md](filter-analysis-2026-09-10.md)。
      **精确拆解（四层相加正好 2,621）**：部门白名单等 `needHandle` **1,326**（50.6%）、
      班级已结课 **990**（37.8%）、无后置课程 **230**（8.8%）、`is_renew_user_clazz=0` **75**（2.9%）。
      Job 今天投递数为 0，那 4 条归因是老师开花名册页面触发的，不是 Job 产出。
      **不是 bug，是预期收窄**；真正的问题是 `success: 2621` 这个汇总具有误导性。
- [ ] 08-28~09-02 那 15,081 行 `handle_status=0` 是否需要补跑（Job 只处理当天创建的记录，不会自动补）
