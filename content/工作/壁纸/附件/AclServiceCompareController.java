package com.gaotu.underlink.app.controller;
...
public class AclServiceCompareController implements ApplicationContextAware {
    ...
    public Map<String, Object> compareService(@RequestBody ServiceCompareRequest request) {
        return getMapRVO(request);
    }

    @PostMapping("/services")
    @ApiOperation(value = "对比服务的不同实现结果", notes = "通过服务名称动态查找实现并调用方法")
    public List<Map<String, Object>> compareServices(@RequestBody ServiceCompareListRequest requests) {
        return requests.getRequests().stream().map(this::getMapRVO).collect(Collectors.toList());

    }

    private Map<String, Object> getMapRVO(ServiceCompareRequest request) {
        if (!enabled) {
            throw new IllegalStateException("ACL服务对比功能未启用，请检查配置");
        }
        try {
            // 1. 查找服务实现类
            ...
            if (strings.length != 2) {
                throw new IllegalArgumentException("serviceNameAndMethodName 格式错误，应为 ServiceName#MethodName");
            }
            ...
            if (implementationsMap.isEmpty()) {
                throw new IllegalArgumentException("未找到服务 " + serviceName + " 的实现类");
            }
            ...
            for (Map.Entry<String, Object> entry : implementationsMap.entrySet()) {
                ...
                try {
                    ...
                    result.put(implName + "_error", e.getMessage());
                }
            }
            if (result.isEmpty()) {
                throw new IllegalStateException("未能成功调用任何实现类的 " + methodName + " 方法");
            }

            return result;

        } catch (Exception e) {
            throw new IllegalArgumentException("对比服务实现时出错: " + e.getMessage(), e);
        }
    }
    ...
    private Map<String, Object> findServiceImplementations(String serviceName, String methodName) {
            ...
            if (serviceImpl != null) {
                ...
                if (serviceV1Impl == null) {
                    ...
                    for (Class<?> interfaceClass : interfaces) {
                        ...
                        // 获取该接口的所有实现类
                        Map<String, ?> beansOfType = applicationContext.getBeansOfType(interfaceClass);
        ...
        if (result.isEmpty()) {
           return getBeanOfTypes(serviceName);
        }
        return result;
    }
     ...
     * 安全获取Bean，不存在时返回null而不是抛出异常
     */
    private  Map<String, Object> getBeanOfTypes(String beanName) {
        try {
            // 加载类
            Class<?> clazz = Class.forName(beanName, false, Thread.currentThread().getContextClassLoader());
            Map<String, ?> beansOfType = applicationContext.getBeansOfType(clazz);
            return (Map<String, Object>) beansOfType;
        } catch (BeansException | ClassNotFoundException e) {
            log.debug("Bean {} 不存在", beanName);
            return null;
        }
                    ...
                    if (params.isEmpty() && paramTypes.length == 0) {
                        ...
                        try {
                            ...
                            if (e instanceof InvocationTargetException) {
                                lastErrorMsg = ((InvocationTargetException) e).getTargetException().getMessage();
                            }
                            try {
                                try {
                                ...
                                if (ex instanceof InvocationTargetException) {
                                    lastErrorMsg = ((InvocationTargetException) ex).getTargetException().getMessage();
                                }
    ...
    private Object convertParameter(Object paramValue, Method method, int paramIndex, Class<?> targetType) {
        ...
        // 1. 如果类型已经匹配，直接返回
        if (targetType.isInstance(paramValue)) {
            return paramValue;
        }

        // 2. 获取方法参数的泛型信息（使用Spring的ResolvableType）
        ResolvableType resolvableType = ResolvableType.forMethodParameter(method, paramIndex);

        log.debug("参数转换 - 索引: {}, 原始类型: {}, 目标类型: {}, 泛型信息: {}",
                paramIndex, paramValue.getClass().getSimpleName(), targetType.getSimpleName(), resolvableType);
    ...
    @Data
    public static class ServiceCompareListRequest {
        private List<ServiceCompareRequest> requests;
    }
} 